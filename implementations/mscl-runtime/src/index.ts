...
import type { ActionProposal, Actor, ProposalRiskLevel, ProposalSource, ProposalTarget } from "./types.js";
...
function isRiskLevel(value: unknown): value is ProposalRiskLevel {
...
function isSource(value: unknown): value is ProposalSource {
  return value === "agent" || value === "user" || value === "system";
}

function validateActionProposal(
  payload: unknown,
): { ok: true; proposal: ActionProposal } | { ok: false; error: string } {
  ...
  const semantic = value.action_proposal as Record<string, unknown> | undefined;
  const raw = semantic?.raw as Record<string, unknown> | undefined;
  const normalized = semantic?.normalized as Record<string, unknown> | undefined;
  ...
  if (
    ...
    return {
      ok: false,
      error:
        "action_proposal.normalized must include category=communication, target, and risk_level.",
    };
  }
...
function startServer(port = 8787, host = "127.0.0.1"): void {

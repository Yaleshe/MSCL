...
test("semantic policy approves valid operator external email", () => {
  ...
  assert.equal(
    decision.policy_evaluations.every((evaluation) => evaluation.result === "pass"),
    true,
  );
});

test("semantic policy rejects viewer sending high-risk external email", () => {
  const decision = evaluateSemanticActionPolicy(
    makeProposal({
  ...
  assert.equal(
    decision.policy_evaluations.some(
      (evaluation) =>
        evaluation.policy === "semantic_action_policy.actor_authorized" &&
...
test("semantic policy rejects mismatched normalized target", () => {
  ...
  assert.equal(
    decision.policy_evaluations.some(
      (evaluation) =>
        evaluation.policy === "semantic_action_policy.normalized_target_matches_raw" &&
        evaluation.result === "fail",
    ),
    true,
  );
